...
class HubView: UIView {
    ...
    private func build() {
        ...
        // FIX: search() was called here on every init — wasted API calls before
        // the user even opened the Hub tab. Now lazy via loadInitialScriptsIfNeeded().
    }
...
extension HubView: UISearchBarDelegate {
    func searchBarSearchButtonClicked(_ searchBar: UISearchBar) {
        ...
        if let query = searchBar.text, !query.isEmpty { search(query: query) }
    }
}

extension HubView: UICollectionViewDataSource, UICollectionViewDelegate {
    func collectionView(_ collectionView: UICollectionView,
                        numberOfItemsInSection section: Int) -> Int { scripts.count }

    func collectionView(_ collectionView: UICollectionView,
                        cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "cell",
                                                      for: indexPath) as! ScriptCardCell
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        NotificationCenter.default.post(
            name: NSNotification.Name("LoadScript"), object: scripts[indexPath.item].script)
    }
}
...
class ScriptCardCell: UICollectionViewCell {
    ...
    private func setup() {
        ...
        // FIX: old code used frame.width in init — frame is .zero at that point,
        // placing the button at x:-60 (invisible). Use constants that match
        // itemSize (280x120); layoutSubviews() corrects dynamically after that.
        titleLabel.frame = CGRect(x: 66, y: 8, width: 204, height: 20)
        titleLabel.font = .systemFont(ofSize: 14, weight: .bold)
        titleLabel.textColor = .white
        contentView.addSubview(titleLabel)

        gameLabel.frame = CGRect(x: 66, y: 28, width: 204, height: 16)
        gameLabel.font = .systemFont(ofSize: 12)
        gameLabel.textColor = .lightGray
        contentView.addSubview(gameLabel)

        loadButton.frame = CGRect(x: 220, y: 40, width: 50, height: 30)
    ...
    override func layoutSubviews() {
        ...
        titleLabel.frame  = CGRect(x: 66, y: 8,  width: bounds.width - 76, height: 20)
        gameLabel.frame   = CGRect(x: 66, y: 28, width: bounds.width - 76, height: 16)
        loadButton.frame  = CGRect(x: bounds.width - 60, y: 40, width: 50, height: 30)
    }
