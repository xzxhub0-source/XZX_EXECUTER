...
static int srfx_getscripts(lua_State *L) { return srfx_getinstances(L); }
static int srfx_getloadedmodules(lua_State *L) { return srfx_getinstances(L); }
...
static int srfx_getrawmetatable(lua_State *L) {
    if (lua_getmetatable(L, 1)) return 1;
    lua_pushnil(L);
    return 1;
}

static int srfx_setrawmetatable(lua_State *L) {
